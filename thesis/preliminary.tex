...
To formalize syntaxes in Agda we use a single data type \Data{Term} indexed by sorts \Sym{s}. 
...
In consequence of using a single data type, we must use extrinsic typing because intrinsically typed terms, for example \Data{Term} \Constr{eₛ} \Constr{⊢} \Data{Term} \Constr{τₛ}, would need to be indexed by themselves and Agda does not support self-indexed data types. 
