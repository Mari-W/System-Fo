...
For example, sort \Constr{eₛ} categorizes expressions \Sym{e}, \Constr{τₛ} categorizes \Sym{τ} and \Constr{κₛ} is used to categorizes the only existing kind \Constr{⋆}. 
...
In consequence we must use extrinsic typing, because intrinsically typed terms \Data{Term} \Constr{eₛ} \Constr{⊢} \Data{Term} \Constr{τₛ} would need to be indexed by themselves and Agda does not allow that. 
