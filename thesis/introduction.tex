...
\end{minted}
We drop the \inl{|\Decl|}expression and transform \inl{|\Inst|}expressions to \inl{let} bindings with unique names. 
...
Invocations of \mono{eq} translate to correct unique variables \mono{eq$_i$} that are bound by the let binding that got introduced for the former resolved instance.
