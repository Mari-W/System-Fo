...
Instantiating a typeclass gives concrete implementations to all the functions defined by the typeclass.
...
Expressions with constraint types \inl{[o : τ] ⇒ τ'} are implicitly treated as expressions of type \Sym{τ'}, given that the constraint \inl{|\Blk{o}| : τ} can be resolved.
...
\noindent A little caveat: the instance for lists would potentially need to recursively call \mono{eq} for sublists but System \Fo's formalization does not actually support recursion. Extending System \Fo\ with recursive let bindings and thus recursive instances is known to be straight forward. 
...
The translation removes all \inl{|\Decl o| in e} expressions. Instance expressions \inl{|\Inst o| = e in e'} are replaced with \inl{let o|$_τ$| = e in e'} expressions, where \mono{o$_τ$} is a unique name with respect to the type \mono{τ} of the expression \mono{e}. 
