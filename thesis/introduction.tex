...
Furthermore, Haskell allows to constrain a type variable \mono{α} via type constraints \inl{Tc α ⇒ τ'} to only be substituted by a concrete type \mono{τ} if there exists an instance \inl{Tc} \mono{τ}. 
...
Next, we instantiate \inl{Eq} for $\{$\mono{α} $\mapsto$ \inl{Nat}$\}$. 
After that, \ \inl{Eq} is instantiated for $\{$\mono{α} $\mapsto$ \inl{[β]}$\}$, given that an instance \inl{Eq β} can be resolved.
Hence we can call \mono{eq} on expressions with type \inl{Nat} and \inl{[Nat]}. 
...
Hence, System \Fo\ is not more expressive or powerful than System F. 
After all, overloading is more of a convenience feature. 
We simply could use let bindings with unique variable names and pass constraints as higher order functions.
...
\noindent The translation removes all \inl{|\Decl o| in e} expressions. 
...
The implicitly resolved constraint in System \Fo\ will be taken as higher-order function argument in System F.
...
We use indices to represent new unique names.
...
The lambda abstraction takes the constraint that was implicitly resolved in System \Fo\ as explicit higher-order function argument.
