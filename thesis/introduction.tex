...
Constraints can be introduced on the expression level using constraint abstractions \inl{|\lambdabar| (o : τ). e'}. A constraint \inl{|\Blk{o}| : τ} requires the type checker to search for an instance for the overloaded variable \mono{o} that has type \mono{τ}. If the constraint cannot be resolved, then the program is invalid.
...
Because \mono{eq$_2$} is invoked for lists of numbers, we must pass the correct instance to eliminate the new higher-order function binding by explicitly passing instance \mono{eq$_1$} as argument.
