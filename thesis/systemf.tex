...
Alternatively, membership proofs can be constructed via the constructor \Constr{there} \Sym{x}, where \Sym{x} is a membership proof for the sublist \Sym{S'} of \Sym{S} that has one element less. 
...
Both abstractions and forall types introduce an additional sort \Constr{eₛ}, or \Constr{τₛ} respectively, to the index \Sym{S} of their corresponding body to account for the additional new binding of a variable.
...
If we want to apply to the renaming as a function or use the function \Data{extᵣ}, the sort argument \Sym{s} can usually be inferred by Agda. 
...
Similar to terms, typing contexts \Sym{Γ} of type \Data{Ctx} \Sym{S} are indexed by a list of bound variables.
...
\Flookup
Inside the case split on variables \Sym{x}, both cases wrap the looked up \Sym{T} in a weakening. 
As a result, \Sym{T} always has index \Sym{S} that aligns with the current required amount of bound variables. 
...
\noindent The reduction rule \Constr{β-let} is equivalent to \Constr{β-λ} and substitutes the value \Sym{e₂} into \Sym{e₁}. 
...
The lemma \Data{⊢σ-preserves} proves that applying a typed substitution to both the term and typing result preserves the typing.
\Fpreserves
