...
\noindent The sort \Constr{oₛ} translates to \Constr{eₛ}. Because we will translate \Constr{decl} expressions to redundant \Constr{let} expressions, all bindings for overloaded variables translate to normal expression bindings in System F.
...
\noindent The case \Sym{Γ} \Constr{▸} \Sym{c} follows analogously, but uses \Data{⊢wk-cstrᵣ} applied to the induction hypothesis instead of \Data{⊢wkᵣ}. Furthermore, the case \Constr{∅} is impossible because there must exist a context item if there exists a variable by definition.
