...
\noindent When we encounter a normal binding in the case \Constr{under-bind}, we wrap the variable in the \Constr{there} constructor to respect the binding.

\noindent In the case \Constr{under-cstr} we again wrap the variable in an additional \Constr{there} that was not present before.
...
Kind \Constr{⋆} translates to its direct counterpart in System F. 
...
We translate the resolved instance to the unique variable in System F using the \Data{o⇝x} function from above.

\noindent Constraint abstractions translate to normal abstractions with an additional new binding. 

\noindent An implicitly resolved constraint translates to an explicit application that passes the resolved instance as argument. We again use function \Data{o⇝x} to translate the resolved instance to the corresponding unique variable. 
...
The untyped translated System \Fo\ term \Data{⊢t⇝t} \Sym{t} is typed in the translated context \Data{Γ⇝Γ} \Sym{Γ} and has the typing result \Data{T⇝T} \Sym{T}.
\DPTTermPres
...
Lemma \Data{⇝-dist-wk} has type \DPTTypePresWk\ and is used to substitute the required equivalence into the translated typing rule \Data{⇝-pres-⊢} \Sym{⊢e₁}.
...
\noindent Implicitly resolved constraints \Constr{⊢⊘} carry the information \Data{o∶τ∈Γ} about the instance that was resolved. In System F the former constraint is explicitly passed as variable pointing to the correct translated instance. 
...
The lemma \Data{⇝-dist-sub-type} follows by straight forward induction over System \Fo\ types as well, except the case for type variables. 
...
To understand why, we at look at the induction case \Constr{⊢extₛ}.
\DPTVarPresSub
...
The cases for \Constr{⊢dropₛ} and \Constr{⊢drop-cstrₛ} are similar to the case \Constr{⊢extₛ} and align with their definition.

\subsubsection{Variables}\hfill\\\\
We first look at the proof for lemma \Data{⇝-pres-lookup}. 
Lemma \Data{⇝-pres-lookup} is proven via induction over the System \Fo\ context \Sym{Γ}. 
\DPTVarPresLookup
...
\noindent The case \Sym{Γ} \Constr{▸} \Sym{c} follows analogous and case \Constr{∅} is impossible because by definition, there must exist a context item if there exists a variable.

\noindent Lemma \Data{⇝-pres-cstr-solve} can proven via induction over the type for resolved constraints \Data{[} \Sym{c} \Data{]∈} \Sym{Γ}. 
