...
The intuition here is that we need the renaming lemma \Data{⇝-dist-ren-type} applied to the typed weakening \Data{⊢wkᵣ} because \Data{extₛ} is defined by weakening the term that result of the substitution \Sym{σ} being applied to the variable \Sym{x}.
...
\noindent The case \Sym{Γ} \Constr{▸} \Sym{c} follows analogously, but uses \Data{⊢wk-instᵣ} applied to the induction hypothesis instead of \Data{⊢wkᵣ} and does not need an inner case split. Furthermore, the case \Constr{∅} is impossible because by definition, there must exist a context item if there exists a variable.
