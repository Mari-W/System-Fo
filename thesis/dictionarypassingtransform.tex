...
\noindent The sort \Constr{oₛ} translates to \Constr{eₛ}. We will translate \Constr{decl} expressions to redundant \Constr{let} expressions later on. In consequence all bindings for overloaded variables translate to normal expression bindings in System F.
...
We apply the correct equality proof to the typing rule \Constr{⊢`o} using lemma \Data{⇝-pres-cstr-solve}.
...
Other than with renamings, the cases for lemma \Data{⇝-dist-sub-var} do not follow directly from the induction hypothesis. 
...
Because the function \Data{lookup} weakens the type \Sym{τ} that is looked up in \Sym{Γ} in both cases \Constr{here} and \Constr{there}, both use lemma \Data{⇝-dist-ren-type} applied to the typed weakening \Constr{⊢wkᵣ} to account for the weakening. 

\noindent The case \Sym{Γ} \Constr{▸} \Sym{c} follows analogously, but uses \Data{⊢wk-instᵣ} applied to the induction hypothesis instead of \Data{⊢wkᵣ}. Furthermore, the case \Constr{∅} is impossible because there must exist a context item if there exists a variable by definition.
...
\DPTOVarPresLookup
Similar to the case split on variables with constructors \Constr{here} and \Constr{there} in case \Sym{Γ} \Constr{▶} \Sym{τ} of lemma \Data{⇝-pres-lookup}, we have the two cases \Constr{here} and \Constr{under-cstr} in lemma \Data{⇝-pres-cstr-solve}. Both cases work similarly to cases in lemma \Data{⇝-pres-lookup}, except that they use \Data{⊢wk-instᵣ} instead of \Data{⊢wkᵣ}.

\noindent Furthermore, we have the case \Constr{under-bind} that works similar to the case \Sym{Γ} \Constr{▸} \Sym{c} and always uses the induction hypothesis applied to \Data{⊢wkᵣ} because we search for a constraint and not a variable.