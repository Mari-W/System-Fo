...
The more general lemma \Data{⊢σ⇝σ·τ⇝τ≡τ⇝σ·τ} follows by straight forward induction over System \Fo\ types as well, except the case for type variables. 
...
The intuition here is that we need the renaming lemma \Data{⊢ρ⇝ρ·τ⇝τ≡τ⇝ρ·τ} because \Data{extₛ} is defined by weakening the term that result of the substitution \Sym{σ} being applied to the variable \Sym{x}.
...
Because the function \Data{lookup} weakens the type \Sym{τ} that is looked up in \Sym{Γ} in both the base case and induction step, both use lemma \Data{⊢ρ⇝ρ·τ⇝τ≡τ⇝ρ·τ} to account for the weakening. 
