...
Hence \Sym{S} must have an additional entry \Constr{eₛ} at the corresponding position to further function as valid index for the translated \Sym{e₁}. Similarly, an additional sort \Constr{eₛ} must be appended to \Sym{S} inside the body of translated constraint abstractions.
...
\noindent We translate typed \Constr{⊢inst} expressions to \Constr{let} expressions that introduce an additional binding not present in System \Fo.
...
The cases for \Constr{⊢dropₛ} and \Constr{⊢drop-cstrₛ} are similar to the case \Constr{⊢extₛ} and also align with their definition.
...
\noindent The case \Sym{Γ} \Constr{▸} \Sym{c} follows analogous and the case \Constr{∅} is impossible because by definition, there must exist a context item if there exists a variable.
